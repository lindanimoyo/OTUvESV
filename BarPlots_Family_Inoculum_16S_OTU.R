...
#run function
bacpercentID <- percentidentified(otu_abundance_taxa2)

bacpercentID

write.csv(bacpercentID, "Figures/otuvzotu_taxonomy/bacpercent_ID_otu_rarefied.csv")
##########################################################################################
...
phylum_abundance_below1 <- ddply(phylum_abundance_over1, c("Site"), summarise, # Subset the phylum with <1% and classify into other phylum
                                 Kingdom_Phylum = "k__Bacteria;p__;f__other families",
                                 mean = 100-sum(mean),
                                 sd = 0,
                                 n = mean(n),
                                 se= 0
...
write.csv(phyla_sorted, "Figures/otuvzotu_taxonomy/16S_Inoculum_phylasorted_rarefied_otu.csv")
fam_barplot1
ggsave(fam_barplot1,filename=paste("Figures/otuvzotu_taxonomy/fambarplot_inoculum_16S_rarefied_otu.pdf"), width=7, height=4.5)
