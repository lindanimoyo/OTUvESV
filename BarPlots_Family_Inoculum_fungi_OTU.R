...
#run function
fungipercentID <- percentidentified(otu_abundance_taxa2)

fungipercentID


write.csv(fungipercentID, "Figures/otuvzotu_taxonomy/fungipercentpercent_ID_otu_rarefied.csv")
##########################################################################################
...
otu_abundance_taxa3$Kingdom_Phylum <- str_c(otu_abundance_taxa3$Kingdom, otu_abundance_taxa3$Phylum,otu_abundance_taxa3$Family, sep=";") #combine kingdom and phylun
...
phylum_over1 <- subset(phylum_abundance2, mean>5 & Kingdom_Phylum !="k__Fungi;p__unidentified;f__unidentified") # Subset the phylum with >5%
...
#give shorter names
levels(phylum_abundance3$Phylum)[3] <- "Ascomycota;f__Dothideomycetes"
...
phyla_sorted$Sitefullnames <- c(rep("Desert",11),rep("Grassland",12),rep("Pine-Oak",12),rep("Subalpine",12),rep("Scrubland",11))
phyla_sorted
...
fam_barplot1<-
  ...
  labs(x="Site", y="Relative abundance (%) OTU", fill="Family") + theme_bw()+
...
write.csv(phyla_sorted, "Figures/otuvzotu_taxonomy/Fungi_Inoculum_familysorted_otu.csv")

ggsave(fam_barplot1,filename=paste("Figures/otuvzotu_taxonomy/fambarplot_inoculum_fungi.pdf"), width=7, height=4.5)
